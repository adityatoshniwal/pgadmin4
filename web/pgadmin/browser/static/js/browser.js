    ...
    onRefreshTreeNode: async function(nodeItem, opts) {
      ...
      if(!nodeObj?.collection_node) {
        ...
        try {
          ...
          this.tree.update(nodeItem, resp.data);
          this.tree.setLabel(nodeItem, {label: resp.data.label});
          this.tree.addIcon(nodeItem, {icon: resp.data.icon});
          this.tree.setId(nodeItem, {id: resp.data.id});
        } catch (error) {
